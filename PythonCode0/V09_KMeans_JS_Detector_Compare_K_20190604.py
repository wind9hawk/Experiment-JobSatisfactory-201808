...
'''
本代码主要用于比较KMeans聚类若选用其他K值，是否对检测结果有影响
选择K=4
...
class KMeans_OCSVM_Predictor():
    ...
    def Auto_KMeans(self):
        ...
        self.K, self.SC, self.K_Pred = V09_K2_KMeans_Module.Auto_K_Choice(self.KMeans_ATF_Feats, 10)
        ...
        self.cluster_centers = Cal_Cluster_CPB(self.KMeans_ATF_Feats, KMeans_LED_Ratios, KMeans_Cluster_MinMax)
        # sys.exit()
        self.center_cpb = self.cluster_centers[0][0] \
                          + self.cluster_centers[1][0] \
                          + self.cluster_centers[2][0] \
                          + self.cluster_centers[3][0]
        self.center_cpb = self.center_cpb / 4.0
    ...
    def OCSVM_Predictor(self):
        ...
        '''
        train_cls_no = []
        for cls_no in range(4):
            if self.cluster_centers[cls_no][0] < self.center_cpb:
                train_cls_no.append(cls_no)
                print '选中群簇有用户： ', len(self.KMeans_Clusters[cls_no]), '\n'
            else:
                continue
        sys.exit()
