...
# Step-3.1: 如果收件人或者发件人超过一个，则分别按照多个联系人计入；
...
UsersDir = RootDir + '\\' + 'CERT5.2-Users-EmailRecords'
EmailDir = RootDir + '\\' + 'CERT5.2-Users-EmailRecords-0.4'
...
for user in os.listdir(UsersDir)[:]:
    ...
    f_eml_feats = open(EmailDir + '\\' + user[:7] + '-feats.csv', 'w')
    ...
    for line in f_eml_lst:
        ...
        if line[-2] == 'Send':
            ...
            if len(line[3].split(';')) > 1:
                # 说明这是目标用户的群发邮件
                # 只关注目标用户发给了谁？
                for usr in line[3].split(';'):
                    send_to_0 = usr[:usr.index('@')]
                    # 最后写入feat时再将email转换成user_id
                    if send_to_0 not in Send_Users:
                        Send_Users.append(send_to_0)
                        Send_Sum = 1.0
                        Send_Avg_Size = float(line[-1])
                        Send_Avg_Attach = 0.0
                        send_feat = []
                        send_feat.append(send_to_0)
                        ...
                        # 定位
                        Index_sender = Send_Users.index(send_to_0)
        ...
        if line[-2] == 'Receive':
            if len(line[6].split(';')) > 1:
                # 说明这是目标用户的群收邮件
                # 只关注谁发给了目标用户？
                for usr in line[6].split(';'):
                    recv_from_0 = usr[:usr.index('@')]
                    if recv_from_0 not in Recv_Users:
                        Recv_Users.append(recv_from_0)
                        # 建立其接收特征
                        Recv_Sum = 1.0
                        Recv_Avg_Size = float(line[-1])
                        Recv_Avg_Attach = 0.0
                        recv_feat = []
                        recv_feat.append(recv_from_0)
                        recv_feat.append(Recv_Sum)
                        recv_feat.append(Recv_Avg_Size)
                        recv_feat.append(Recv_Avg_Attach)
                        Recv_Feats.append(recv_feat)
                        print user, 'receive email from ', recv_from_0, '分析完毕...\n\n'
                        continue
                    else:
                        # 说明之前已经初始化针对该用户的接收特征，接下来需要更新部分特征参数
                        # 定位
                        Index_recv = Recv_Users.index(recv_from_0)
