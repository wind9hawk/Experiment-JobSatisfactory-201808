# coding:utf-8
# 本实验基于检测CERT5.2中Leave Users的按月动态检测，需要替换每月的ATF特征与离职用户特征，其他不变
# 注意：
# 月度特征ATF默认包含了OCEAN，需要事先提取时去掉
# 为了比较与Static整体的Recall与FPR，同时输出每次检测出的月度insiders个数与离职中的误报个数
...
class KMeans_OCSVM_Predictor():
    def __init__(self, dst_dir, month):
        self.Month = month
        self.Dst_Dir = dst_dir
        self.Analyze_Path = os.path.dirname(self.Dst_Dir)
        # 初始化JS特征
        ...
        print 'JS_lst 初始化完毕..\n'
        f_ATF = open(self.Dst_Dir + '\\' + 'CERT5.2_Leave_ATF_02.csv', 'r')
        ...
        print 'ATF_lst 初始化完毕..\n'
        self.Month_Users_Length = len(self.ATF_lst)
        # sys.exit()
        # 所有初始ATF特征均标准
        # 初始化Leave Users
        self.Leave_Users = []
        # Leave_Users_2010-04.csv
        f_leave = open(self.Dst_Dir + '\\' + 'Leave_Users_' + self.Month + '.csv', 'r')
        ...
        # 初始化Insiders列表
        # insider, leave_month
        f_insider_1 = open(self.Analyze_Path + '\\' + 'Insiders-1_Leave.csv', 'r')
        f_insider_2 = open(self.Analyze_Path + '\\' + 'Insiders-2_Leave.csv', 'r')
        f_insider_3 = open(self.Analyze_Path + '\\' + 'Insiders-3_Leave.csv', 'r')
        # Extract_Lines函数返回的是特征数值，不包含用户名
        self.Insiders_1 = []
        self.Insiders_2 = []
        self.Insiders_3 = []
        self.Month_Insiders_Length = 0
        self.Month_Insiders_1 = []
        self.Month_Insiders_2 = []
        self.Month_Insiders_3 = []
        ...
        for line in f_insider_1.readlines():
            ...
            if line_lst[0] in self.Leave_Users:
                self.Month_Insiders_Length += 1
                self.Month_Insiders_1.append(line_lst[0])
        f_insider_1.close()
        ...
        for line in f_insider_2.readlines():
            ...
            if line_lst[0] in self.Leave_Users:
                self.Month_Insiders_Length += 1
                self.Month_Insiders_2.append(line_lst[0])
        f_insider_2.close()
        ...
        for line in f_insider_3.readlines():
            ...
            if line_lst[0] in self.Leave_Users:
                self.Month_Insiders_Length += 1
                self.Month_Insiders_3.append(line_lst[0])
        f_insider_3.close()
    ...
    def Auto_KMeans(self):
        ...
        # 先保存JS的聚类结果
        f_kmeans_pred = open(self.Dst_Dir + '\\' + self.Month + '_CPB_KMeans_Pred.csv', 'w')
    ...
    def OCSVM_Predictor(self):
        ...
        for nu_0 in nu_lst:
            ...
            # sys.exit()
            cnt_recall, recall, cnt_fp, fpr_total, fpr_leave, tp_index, recall_index, fp_index = Cal_Messures(
                                                                                                self.Labels,
                                                                                                pred)
            ...
            for index_i in self.Test_Insiders_3_Index:
                if pred[index_i] == 1:
                    recall_3 += 1
            if len(self.Month_Insiders_1) == 0 :
                recall_1 = None
            else:
                recall_1 = recall_1 / len(self.Month_Insiders_1)
            if len(self.Month_Insiders_2) == 0 :
                recall_2 = None
            else:
                recall_2 = recall_2 / len(self.Month_Insiders_2)
            if len(self.Month_Insiders_3) == 0 :
                recall_3 = None
            else:
                recall_3 = recall_3 / len(self.Month_Insiders_3)

            #ocsvm_tmp.append(recall_1 / len(self.Month_Insiders_1))
            #ocsvm_tmp.append(recall_2 / len(self.Month_Insiders_2))
            #ocsvm_tmp.append(recall_3 / len(self.Month_Insiders_3))
            ocsvm_tmp.append(recall_1)
            ocsvm_tmp.append(recall_2)
            ocsvm_tmp.append(recall_3)
...
for month in os.listdir(Dst_Dir):
    if os.path.isdir(Dst_Dir + '\\' + month) == True and 'KMeans' not in month:
        Months_lst.append(month)
    else:
        continue
print 'CERT5.2 月份列表：', Months_lst, '\n'
print '开始逐月检测:..\n'
for month in Months_lst[5:6]:
    dst_dir = Dst_Dir + '\\' + month
    # 初始化对象
    Predictor_obj = KMeans_OCSVM_Predictor(dst_dir, month)
